...
extern int isSharedHTM;
extern volatile __thread int crafty_isValidate;
extern __thread long nbTransactions;
extern volatile __attribute__((aligned(ARCH_CACHE_LINE_SIZE))) uint64_t* htReadClockVal;
extern uint64_t ht_add_count;
...
#define BEFORE_SGL_COMMIT(tid) \
  if (on_before_sgl_begin != NULL) { \
      on_before_sgl_begin(tid); \
  }
...
void install_bindings_pcwm_eadr();
...
void state_gather_profiling_info_pcwm_eadr(int threadId);
...
void state_fprintf_profiling_info_pcwm_eadr(char *filename);
...
void on_before_htm_begin_pcwm_eadr(int threadId);
...
void on_htm_abort_htmUndo(int threadId);
...
void on_htm_abort_pcwm_eadr_t2(int threadId);
...
void on_htm_abort_pcwm_eadr(int threadId);
...
void on_before_htm_write_8B_pcwm_eadr(int threadId, void *addr, uint64_t val);
...
void on_before_htm_non_tx_write (int threadId, void *addr, uint64_t val);

void on_before_htm_commit_pc(int threadId);
void on_before_htm_commit_htmOnly(int threadId);
void on_before_htm_commit_htmUndo(int threadId);
...
void on_before_htm_commit_pcwm_eadr(int threadId);
...
void on_after_htm_commit_pcwm_eadr(int threadId);
...
void wait_commit_htmUndo(int threadId);
...
void wait_commit_pcwm_eadr_t1(int threadId);
void wait_commit_pcwm_eadr_t2(int threadId);
...
void wait_commit_pcwm_eadr(int threadId);
...
#define LOOK_UP_FREE_SLOT(_tid) ({ \
  ...
  for (int i = 0; i < gs_appInfo->info.nbThreads; ++i) { /* finds the maximum */ \
...
#define FIND_LAST_SAFE_EPOCH() ({ \
  ...
  for (int _i = 1; _i < gs_appInfo->info.nbThreads; ++_i) { \
