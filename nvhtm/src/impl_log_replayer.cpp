...
/* extern */ volatile __attribute__((aligned(ARCH_CACHE_LINE_SIZE))) int P_startIdx = 0; /* persistent */
...
static inline void BITSET_INIT()
{ // TODO: put on a MACRO
...
static void *sorter_fn(void *arg)
{
  ...
  int isDone = 0;
  while (!__atomic_load_n(&replay_is_ready, __ATOMIC_ACQUIRE))
    usleep(10);
  // sleep(1);
  ...
  do
  {
    ...
    else if ((flags & LOG_REPLAY_FORWARD)) // RP ENTERS HERE
    {
    ...
    do
    {
      ...
      else if ((flags & LOG_REPLAY_FORWARD) && (flags & LOG_REPLAY_PHYSICAL_CLOCKS)) // RP CHECK FOR LOG_REPLAY_PHYSICAL_CLOCKS
      {
...
static inline void fix_next_ptr(int replayerId)
{
  ...
  {
    // printf("IN IF FIX_NEXT_PTR\n");
    if (nbReplayers <= 1)
    {
      // printf("IN nbReplayers<=1 FIX_NEXT_PTR\n");
      for (int t = 0; t < nbThreads; ++t)
      {
        if (curPtrs[t] == (uint64_t)-1L)
          continue;
...
/*
 * This function will update all timestamps for each log of the worker
 * threads (curTSs[]).
 *
 * The return value is -1 if no valid timestamp was found or the thread id of
 * the log with the oldest timestamp.
 */
static int get_current_thread_whom_log_need_to_be_replayed()
{
  uint64_t curTs = (uint64_t)-1;
  int curThread = -1;

  for (int i = 0; i < nbThreads; ++i)
  {
    // get the last index of the log applied for this thread 
    int start = curPtrs[i]; 

    // get the last entry idx in the log
    ...
    // the log is not empty - we need to advance until we see a valid commit marker or 
    // we reach the end of on-going log (worker thread is still building it)
    while (!isBit63One(__atomic_load_n(&writeLog[i][start], __ATOMIC_ACQUIRE)) && start != end)
    {
      // advance to the next log entry
      // NOTE: advancing in pairs (+2) like in the original code might cause a
      // problem (albeit in very rare situations) in which the worker thread
      // wrote the address but not the value (we might read an invalid log
      // entry and assume it is a timestamp)
      start = (start + 1) & (logSize - 1);
      // keep in mind that the worker threads might still advancing the next
      // idx
      end = __atomic_load_n(&logPtrs[i].log_ptrs.write_log_next, __ATOMIC_ACQUIRE);
    }

    // if we found a complete log ending with a commit marker, start has its
    // index - otherwise the log is not complete and we stop
    // NOTE: what if the log is incomplete and we read an invalid entry and
    // interpret it as a valid timestamp???? not sure if that can happen
    if (!isBit63One(__atomic_load_n(&writeLog[i][start], __ATOMIC_ACQUIRE)))
      continue;

    // update the current timestamp of the log of that particular thread (the
    // marker bit is reset)
    uint64_t ts = zeroBit63(writeLog[i][start]);
    curTSs[i] = ts;

    // if we have found an older timestamp, updated it as the current one
 ...
 *
 */
static void apply_log(int curThread)
{
  uint64_t res = -1;
 
  // start index of the log to be applied
  int idx = curPtrs[curThread];
  //int idx = curPtr.idx;
  
  // keep going in the log until we reach the marker - apply each entry 
  while (!isBit63One(writeLog[curThread][idx]))
  {
    // assumes <addr,val>
    uintptr_t addr = writeLog[curThread][idx];
    uintptr_t val = writeLog[curThread][(idx + 1) & (logSize - 1)];
    ...
    *((uintptr_t *)addr) = val;
    ...
    //printf("Replayer: Applied -- addr: %lx - val: %lx\n", addr, *((uintptr_t *)addr));
  
    // This is really weird - we should not reach this case as the log will
    // necessarily end with a commit marker
  ...
  // make the previous flushes visible to other threads
  FENCE_PREV_FLUSHES(); 

  // now we need to update some variables 

  // this is the new starting log index for this thread
  curPtrs[curThread] = (idx+1) & (logSize - 1);
  // and this the timestamp
  curTSs[curThread] = zeroBit63(writeLog[curThread][idx]);

  // updates the start index of the log so that the worker threads can reuse
  // log space
  __atomic_store_n(&logPtrs[curThread].log_ptrs.write_log_start, 
                   curPtrs[curThread], __ATOMIC_RELEASE); 
  // printf ("apply_log writeLogStart: %ld\n", logPtrs[curThread].log_ptrs.write_log_start);
 ...
 * Main structures:
 *
 * curPtrs[nbThreads]: contains the last idx of the log applied for each
 *                     thread
 * curTSs[nbThreads]:  contains the last log's timestamp for a given thread
 *
 * logPtrs[nbThreads].log_ptrs.write_log_next:
 *                     this is the counter that is incremented by the worker
 *                     threads and points to the last entry in the log;
 *                     we use it as an indicator of the last entry in the log
 *                     but we must avoid writing directly to it (the first
 *                     timestamp entry prior to this entry is safe to be
 *                     manipulated by the replayer)
 *
 * curTs:  current timestamp not yet applied
 * curThread:  current thread whom the log will be replayed
 *
 *   }
 */
static void new_single_replayer_apply()
{
  
  uint64_t curTs = -1;
  int curThread = -1;
  
  // Not sure if this necessary - just keeping from the old code since it
  // appears critical (commentary below is from the original code)
  // TODO: replace extern with better solution
  // This is a quick hack to solve an issue with latestSafeTsv having a NULL value during execution
  if (latestSafeTsv == NULL)
  {
    extern uint64_t **G_flag_checkpointer_P_last_safe_ts;
    latestSafeTsv = *G_flag_checkpointer_P_last_safe_ts;
  }
  ...
  while (1) 
  {
    ...
    if (isExit) {
      // we know the app has quit, but there might still be logs to be replayed,
      curThread = get_current_thread_whom_log_need_to_be_replayed();
 
      // no more logs, we are safe to quit
      if (curThread == -1) break;
    } 
    else curThread = get_current_thread_whom_log_need_to_be_replayed();

    // still no log, keep trying to get a valid one
    if (curThread == -1) continue;

 
    // we have found a valid log, so apply it
    apply_log(curThread);

    // update the most recent timestamp replayed
    htReadClockVal = (uint64_t*) ((uint8_t*)G_flag_checkpointer_exit + sizeof(uint64_t) * 1024);
    // only updates when the log's timestamp is newer (greater)
    if (curTSs[curThread] > *htReadClockVal)
      __atomic_store_n(htReadClockVal, curTSs[curThread],__ATOMIC_RELEASE); //loop is always on, here is the closest to the commit

  //  printf ("REPLAYER:  htReadClockVal %lx\n", *htReadClockVal);
  }
}
...
//  htReadClockVal = (uint64_t*) ((uint8_t*)G_flag_checkpointer_exit + sizeof(uint64_t) * 4); //there are exit variables
//  __atomic_store_n(htReadClockVal,rdtscp(),__ATOMIC_RELEASE); //loop is always on, here is the closest to the commit
//    __atomic_store_n(htReadClockVal,0,__ATOMIC_RELEASE); //loop is always on, here is the closest to the commit
//  printf("Entrando no laço principal:  htReadClockVal %lx\n", *htReadClockVal); fflush(stdout);
  do
  {
    int isExit = __atomic_load_n(G_flag_checkpointer_exit, __ATOMIC_ACQUIRE);
    ...
    {
      //const int MIN_SPACE_IN_LOG = 16; // wait to start replaying after there's 64 entries
      const int MIN_SPACE_IN_LOG = 4; // wait to start replaying after there's 64 entries
      long totSize = gs_appInfo->info.allocLogSize;
      ...
      for (int i = 0; i < nbThreads; ++i)
      {
        ...
        //printf("REPLAYER FLAG: size = %ld | min_space = %ld\n",
        //  size, MIN_SPACE_IN_LOG);
    ...
    {
      //printf("Antes do search_pc\n"); fflush(stdout);
      tx = replay_log_nextTx_forward_search_pc();
      //printf("Replayer pegou um log: %d\n", tx);
    }
    else if ((flags & LOG_REPLAY_BACKWARD) && (flags & LOG_REPLAY_LOGICAL_CLOCKS))
    {
    ...
    if ((flags & LOG_REPLAY_CONCURRENT) && (tx.idx == -1) && isExit /* && !isRepeat*/)
    {
      ...
      for (int i = 0; i < nbThreads; ++i)
      {
        if (__atomic_load_n(&logPtrs[i].log_ptrs.write_log_next, __ATOMIC_ACQUIRE) != __atomic_load_n(&logPtrs[i].log_ptrs.write_log_start, __ATOMIC_ACQUIRE))
        {
          safeExit = 0;
          break;
        }
...
//        printf("log replay forward\n");
    ...
    if ((flags & LOG_REPLAY_CONCURRENT) && (flags & LOG_REPLAY_FORWARD) && concWBINVD > writesThreshold)
    {
    ...
    if ((flags & LOG_REPLAY_CONCURRENT) && (tx.idx == -1) && !isExit)
    {
    ...
    else if ((flags & LOG_REPLAY_BUFFER_FLUSHES) || (flags & LOG_REPLAY_BUFFER_WBINVD) || (flags & LOG_REPLAY_RANGE_FLUSHES))
    {
    ...
    //printf("Replayer bef hash\n");
    #if defined(USE_HASHMAP) || defined(USE_SIMPLEHASH)
      htReadClockVal = (uint64_t*) ((uint8_t*)G_flag_checkpointer_exit + sizeof(uint64_t) * 1024);
      //__atomic_store_n(htReadClockVal, *latestSafeTsv,__ATOMIC_RELEASE); //loop is always on, here is the closest to the commit
      __atomic_store_n(htReadClockVal, curTSs[curThread],__ATOMIC_RELEASE); //loop is always on, here is the closest to the commit
  ...
  printf("replayerStats\n\t%d %d\n", startCounter, replayActCounter); // startCounter: // replayActCounter:
}
...
extern "C"
{
  void *internal_replay_log_apply_fn(void *replayerId)
  {
    ...
    // if (stop_replayer == 1)
    // {
    //   replay_is_ready = 0; // enables back by calling replay_log_apply();
    // }

    while (!__atomic_load_n(&replay_is_ready, __ATOMIC_ACQUIRE))// && stop_replayer==1)
      usleep(10);
    // sleep(1);
    snapshotPtrs = (uint64_t *)malloc(nbThreads * sizeof(uint64_t));
    curPtrs = (uint64_t *)malloc(nbThreads * sizeof(uint64_t));
    ...
    printf ("curPtrs: %d\n");
    curTSs = (uint64_t *)malloc(nbThreads * sizeof(uint64_t));
    // printf("SPtrs: %lx, curPtrs: %lx, curPtrsThrs[arg]: %lx, curTSs: %lx\n", snapshotPtrs, curPtrs, curPtrsThrs[arg], curTSs);
    // see which var is the max log size, so we can define the % to set replay_is_ready to 1
...
static inline replay_log_next_entry_s replay_log_search_cc_htm()
{
  ...
  procRedIdx = ri;
  if (CC_HTM_Q->entries[ri].endTS > 0 && CC_HTM_Q->entries[ri].logPtr != 0 && CC_HTM_Q->entries[ri].endTS <= UpdatedBlueHead())
  {
...
static inline replay_log_next_entry_s replay_log_nextTx_forward_search_pc()
{
  ...
  while (start != logPtrs[curThread].log_ptrs.write_log_next && !isBit63One(writeLog[curThread][start]))
  {
...
static inline replay_log_next_entry_s replay_log_nextTx_backward_search_lc()
{
  ...
  while ((int)((end + logSize - 1) & (logSize - 1)) != logPtrs[curThread].log_ptrs.write_log_start && !isBit63One(writeLog[curThread][end]))
  {
...
static inline void replay_log_backward_find_maximum_target_ts()
{
  ...
  for (int i = 0; i < nbThreads; ++i)
  {
    ...
    if (flags & LOG_REPLAY_PHYSICAL_CLOCKS_SORTED_BACKWARD)
    {
      ...
      while (end != logPtrs[curThread].log_ptrs.write_log_start && !isBit63One(writeLog[curThread][end]))
      {
...
static inline replay_log_next_entry_s replay_log_nextTx_forward_search_pc_sorted(int useHints, int updateCurTx)
{
  if (curTx.threadId == -1 || !useHints)
  { // first TX, must lookup
    if ((flags & LOG_REPLAY_CONCURRENT))
    {
      replay_log_forward_find_minimum_target_ts();
    }
    ...
    for (int i = 0; i < nbThreads; ++i)
    {
      //// ----
      int start = curPtrs[i];
      if (start == logPtrs[i].log_ptrs.write_log_next)
        continue;
      nextIdx = start;
  ...
  { // we know the next thread
    int startThread = curTx.threadId;
    P_startIdx = curTx.idx;
    replay_log_next_entry_s res = (replay_log_next_entry_s){startThread, (int)((P_startIdx + 1) & (logSize - 1))};

    if (!isBit63One(writeLog[startThread][P_startIdx]))
    {
    ...
    {
      uint64_t nextTxInfo = zeroBit63(writeLog[startThread][P_startIdx]);
      curTx.threadId = nextTxInfo >> 32;
      curTx.idx = nextTxInfo & 0xFFFFFFFF;
    }
    // printf("startThread %d, P_startIdx %d\n", startThread, P_startIdx);
    return res;
  }
  return (replay_log_next_entry_s){-1, -1};
}
...
static inline replay_log_next_entry_s replay_log_nextTx_backward_search_pc_sorted(int useHints, int updateCurTx)
{
  ...
  { // we know the next thread
    ...
    int P_startIdx = curTx.idx;
    int endIdx = P_startIdx;
...
static inline int check_apply(uintptr_t addr, int replayerId)
{
  ...
  if ((nbReplayers <= 1 || !(flags & LOG_REPLAY_PARALLEL)) && ((addr >= (uintptr_t)nvramRanges[0] && addr < (uintptr_t)nvramRanges[1]) // socket 0
                                                               ||
...
static inline uint64_t replay_log_applyTx_andAdvance(replay_log_next_entry_s curPtr, int replayerId)
{
  uint64_t res = -1;
  int idx = curPtr.idx;
  float logSizeThreshold = logSize * 0.8;
  // if (Paging.memory_heap_mmap == 0)
  // {                                        // cheat to make the replay mem addr offset, based on hard coded SIZE_OF_PRIV in tm.h
  //   Paging.memory_heap_mmap = 2147483648L; // 4294967296L; // 4 * 1024 * 1024 * 1024;
  // }
  // printf("LOG_SIZE_THRESHOLD %d\n", logSizeThreshold);
  while (!isBit63One(writeLog[curPtr.threadId][idx]))
  {
    ...
    analizeWrites++; // compare this with size of log
    #ifdef USE_PAGING
//      addr += Paging.memory_heap_mmap;
//      countWrites++;
    #endif
//    printf("Replayer: To be applied -- addr: %lx - val: %lx\n", addr, val);
    *((uintptr_t *)addr) = val;
    printf("Replayer: Applied -- addr: %lx - val: %lx\n", addr, *((uintptr_t *)addr));
    // if (iz < 15)
    // {
    // printf("2addr: %p || val: %lx || mhm: %llu\n", addr, val, Paging.memory_heap_mmap);
    //   iz++;
    // }
    if (idx == logPtrs[curPtr.threadId].log_ptrs.write_log_next)
      break;
  ...
  // if (analizeWrites >= logSizeThreshold && (waitForReplayer == 0 || waitForReplayer == 1))
  // {
  //   waitForReplayer = 1;
  // }
  // else
  // {
  //   if (waitForReplayer != 2)
  //     waitForReplayer = 0;
  //   // printf("waitforreplayer0\n");
  // }
  if (idx != logPtrs[curPtr.threadId].log_ptrs.write_log_next)
  {
...
static inline uint64_t replay_log_applyTx_backward(replay_log_next_entry_s curPtr, int replayerId)
{
  ...
  while (!isBit63One(writeLog[curPtr.threadId][idx]))
  { // looks for the other TS (beginning of TX)
    // passes all the writes ands goes to the TSs (assumes <addr,val>, first bit of addrs is 0)
    ...
    if ((int)idx == (int)logPtrs[curPtr.threadId].log_ptrs.write_log_start)
      break; // for the linking
  ...
  if ((int)((idx + 1) & (logSize - 1)) == (int)logPtrs[curPtr.threadId].log_ptrs.write_log_start && idx != logPtrs[curPtr.threadId].log_ptrs.write_log_start)
  {
    res = zeroBit63(writeLog[curPtr.threadId][idx]);
  }
  // idx = (idx + 1) & (logSize - 1); // <commit marker>
