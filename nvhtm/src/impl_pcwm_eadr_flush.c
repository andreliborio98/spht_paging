...
void on_htm_abort_pcwm_eadr_t5(int threadId) { /* empty */ }
...
static inline void smart_close_log_pcwm(uint64_t marker, uint64_t *marker_pos)
{
  ...
  if (firstCL == lastCL) {
    // same cache line
    // The full log + commit marker fits in cache
    // There won't be partial log writes in PM in case of a a cache line eviction
    // *marker_pos = marker; // No longer required since it was already written on_before_htm
    ...
    // FENCE_PREV_FLUSHES(); // Might not be required
    // *marker_pos = marker; // No longer required since it was already written on_before_htm
...
void on_after_htm_commit_pcwm_eadr_t5(int threadId)
{
    ...
    if (writeLogStart != writeLogEnd) {
        ...
        // Smart close log might not be required since the timestamp marker was written on_before_htm_commit_pcwm_eadr_t5
...
void wait_commit_pcwm_eadr_t5(int threadId) { /* empty */ }
