...
public:
        ...
        __attribute__((transaction_safe)) void insert(KEY key, VALUE value) {
...
__attribute__((transaction_safe))  bool find(const KEY& key, VALUE* value= 0) const {
...
__attribute__((transaction_safe)) bool del(const KEY& key) {
  InnerNode* inner;
...
__attribute__((transaction_safe)) bool findLastLessThan(const KEY& key, VALUE* value = 0, KEY* out_key = 0) const {
...
private:
	  ...
	  //                unsigned char _pad[LEAF_NODE_PADDING];
        ...
        __attribute__((transaction_safe)) bool leaf_insert(LeafNode* node, KEY& key,
        ...
        __attribute__((transaction_safe))static void leaf_insert_nonfull(LeafNode* node, KEY& key, VALUE& value,
                        unsigned index) {
        ...
        __attribute__((transaction_safe)) bool inner_insert(InnerNode* node, unsigned current_depth, KEY& key,
                        VALUE& value, InsertionResult* result) {
        ...
        __attribute__((transaction_safe)) void inner_insert_nonfull(InnerNode* node, unsigned current_depth, KEY& key,
                        VALUE& value) {
                // Simple linear search
