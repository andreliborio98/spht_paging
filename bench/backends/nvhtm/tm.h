...
#define SIZE_OF_PRIV (3 * 1024 * 1024 * 1024L)   // this is the size of memory each thread will have for itself
#define SIZE_OF_SHARED (8 * 1024 * 1024 * 1024L) // 16MB
...
#define TM_STARTUP(numThread)                                      \
  ...
  global_structs_init(                                             \
      ...
      32*1024*1024L /*268435456L /* LOG_SIZE: in nb of entries */,                 \
      SIZE_OF_PRIV /*6442450944L /* 3221225472L /* 12884901888L 8589934592L LOCAL_MEM_REGION */, \
      SIZE_OF_SHARED /*8589934592L /* SHARED_MEM_REGION */,                         \
...
#define TM_THREAD_EXIT()                                                                             \
  ...
  __atomic_fetch_add(&stats_nbSuccess, HTM_get_status_count(HTM_SUCCESS, NULL), __ATOMIC_RELAXED);   \
  ...
  __atomic_fetch_add(&stats_nbExpli, HTM_get_status_count(HTM_EXPLICIT, NULL), __ATOMIC_RELAXED);    \
  __atomic_fetch_add(&stats_nbOther, HTM_get_status_count(HTM_OTHER, NULL), __ATOMIC_RELAXED);       \
...
extern int isSharedHTM; // need flag for HTM with shared memory pool
#define TM_BEGIN()          \
...
#define PM_PRIVATE_WRITE(var, val) ({ onBeforeWrite(HTM_SGL_tid, &var, val); var = val; var; })
...
static void input_handler()
{
  ...
  if (input_exists("LOG_REPLAY_CONCURRENT"))
  {
  ...
  else if (input_exists("usePCWMeADRT1"))
  {
  ...
  else if (input_exists("usePCWMeADRT2"))
  {
  ...
  else if (input_exists("usePCWMeADRT3"))
  {
  ...
  else if (input_exists("usePCWMeADRT4"))
  {
  ...
  else if (input_exists("usePCWMeADRT5"))
  {
