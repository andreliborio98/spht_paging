...
title_map = {
    ...
    "STOCKLVnORDSTATUS": "StockLv+OrderStatus",
    "ORDSTATUSnPAY": "OrderStatus+Payment",
    "ORDSTnPAYn50DEL": "OrdSt+Paym+Delivery",
    "NEWORDnORDSTATUS": "NewOrder+OrderStatus",
    "A": "Workload A (TPCC Docs)",
}
...
for i in range(0, len(files), 2):
    ...
    with open(files[i], "r") as avgFile:
        with open(files[i + 1], "r") as stdevFile:
            ...
            # if "PSTM" in titles[i]:
            #     marker = "p"
            if "DELIVERY" in titles[i]:
                marker = "^"
            if "NEWORDER" in titles[i]:
                marker = "+"
            if "PAYMENT" in titles[i]:
                marker = "x"
            if "STOCKLEVEL" in titles[i]:
                marker = "1"
            if "ORDERSTATUS" in titles[i]:
                marker = "2"
            if "STOCKLVnORDSTATUS" in titles[i]:
                marker = "p"
            if "ORDSTATUSnPAY" in titles[i]:
                marker = "v"
            if "NEWORDnORDSTATUS" in titles[i]:
                linewidth = 1.3
