...
fig, axs = plt.subplots(
    nrows=1, ncols=1, figsize=(7, 2.5), sharex=False, gridspec_kw={"hspace": 0.1}
)
        ...
        with open(files[i + 1], "r") as stdevFile:
            ...
            for avg, stdev in zip(avgs, stdevs):
                ...
                if "PSTM" in titles[i]:
                    ...
                    sgl_commits_err = np.append(
                        sgl_commits_err, float(stdev[2]) - float(stdev[2])
                    )
            ...
            if not showFirstAbort:
                ...
                axs.bar(
                    ...
                    htm_commits,
                    bar_width,
                    bottom=aborts + sgl_commits,
                    yerr=htm_commits_err,
                    label="HTM",
                    hatch="//",
                    color=color_str,
                    edgecolor="black",
...
plt.xticks(
    np.array(ind_ticks).flatten(), np.array(ind_label).flatten(), rotation=75, size=1
)
...
plt.text(1.15, -0.83, "2 threads")
plt.text(2.15, -0.83, "4 threads")
plt.text(3.15, -0.83, "8 threads")
plt.text(4.15, -0.83, "12 threads")
plt.text(5.15, -0.83, "16 threads")
plt.text(6.15, -0.83, "20 threads")
plt.text(7.15, -0.83, "24 threads")
