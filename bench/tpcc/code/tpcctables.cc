...
__attribute__((transaction_safe)) void * tm_memcpy(void *dst, const void *src, size_t len)
{
        ...
        len % sizeof(long) == 0)
    {
        ...
        for (i=0; i<len/sizeof(long); i++) {
            //d[i] = s[i];
            FAST_PATH_SHARED_WRITE(d[i], s[i]);
        }
    }
    else {
        char *d = (char*)dst;
...
/*    __transaction_atomic { */
  ...
  for (int64_t order_id = o_id - STOCK_LEVEL_ORDERS; order_id < o_id; ++order_id) {
    for (int line_number = 1; line_number <= Order::MAX_OL_CNT; ++line_number) {
      ...
      stock_quantity = TM_SHARED_READ(stock->s_quantity);
	...
	if (stock_quantity > 0) {
    TM_SHARED_WRITE(stock->s_w_id,1);
    // if (local_exec_mode == 1 || local_exec_mode == 3){
...
__attribute__((transaction_safe)) void TPCCTables::internalOrderStatus(TM_ARGDECL Customer* customer, OrderStatusOutput* output) {
  ...
  if (customer->c_id > 2){
    ...
    output->c_balance = TM_SHARED_READ_D(customer->c_balance);
    // if(local_exec_mode == 1 || local_exec_mode == 3){
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderRemote(
  ...
  for (int i = 0; i < getVectorSize(items); ++i) {
    ...
    intptr_t temp_var = TM_SHARED_READ(stock->s_quantity);
    if (temp_var >= item.ol_quantity + 10) {
      TM_SHARED_WRITE(stock->s_quantity, temp_var-item.ol_quantity);
    } else {
      TM_SHARED_WRITE(stock->s_quantity, temp_var-item.ol_quantity+91);
    }
    intptr_t temp_var_2 = TM_SHARED_READ(stock->s_ytd);
    TM_SHARED_WRITE(stock->s_ytd, temp_var_2+item.ol_quantity);
    intptr_t temp_var_3 = TM_SHARED_READ(stock->s_order_cnt);
    TM_SHARED_WRITE(stock->s_order_cnt, temp_var_3+1);
    // newOrderHome calls newOrderRemote, so this is needed
    if (item.ol_supply_w_id != home_warehouse) {
        // remote order
        intptr_t temp_var_4 = TM_SHARED_READ(stock->s_remote_cnt);
        TM_SHARED_WRITE(stock->s_remote_cnt, temp_var_4+1);
    }
    // if(local_exec_mode == 1 || local_exec_mode == 3){
...
__attribute__((transaction_safe)) void TPCCTables::paymentHome(TM_ARGDECL int64_t warehouse_id, int64_t district_id, int64_t c_warehouse_id,
    ...
    // }
    float temp_float = TM_SHARED_READ_D(w->w_ytd);
    TM_SHARED_WRITE_D(w->w_ytd, temp_float+h_amount);
    District* d = findDistrict(TM_ARG warehouse_id, district_id);
    float temp_float_2 = TM_SHARED_READ_D(d->d_ytd);
    TM_SHARED_WRITE_D(d->d_ytd, temp_float_2+h_amount);
...
__attribute__((transaction_safe)) void TPCCTables::internalPaymentRemote(
  ...
  float temp_float = TM_SHARED_READ_D(c->c_balance);
  TM_SHARED_WRITE_D(c->c_balance, temp_float-h_amount);
  float temp_float_2 = TM_SHARED_READ_D(c->c_ytd_payment);
  TM_SHARED_WRITE_D(c->c_ytd_payment, temp_float_2+h_amount);
  intptr_t temp_var = TM_SHARED_READ(c->c_payment_cnt);
  TM_SHARED_WRITE(c->c_payment_cnt, temp_var+1);

  output->c_credit_lim = TM_SHARED_READ_D(c->c_credit_lim);
  output->c_discount = c->c_discount;
  output->c_balance = TM_SHARED_READ_D(c->c_balance);
  // if(local_exec_mode == 1 || local_exec_mode == 3){
...
/*    __transaction_atomic { */
  ...
  for (int64_t d_id = 1; d_id <= District::NUM_PER_WAREHOUSE; ++d_id) {
    ...
    float temp_float = TM_SHARED_READ_D(c->c_balance);
    TM_SHARED_WRITE_D(c->c_balance, temp_float+total);
    intptr_t temp_var = TM_SHARED_READ(c->c_delivery_cnt);
    TM_SHARED_WRITE(c->c_delivery_cnt, temp_var+1);
    // if (local_exec_mode == 1 || local_exec_mode == 3){
    //   float temp_float = SLOW_PATH_SHARED_READ_D(c->c_balance);
...
__attribute__((transaction_safe)) static T* insert(
  ...
  // if (freedValues[sizeof(T)].empty()) {
  if (TM_isSequential) {
    copy = (T*) P_MALLOC_THR(sizeof(T), TM_thrMallocCnt);
    TM_thrMallocCnt = (TM_thrMallocCnt + 1) % TM_totNbThreads;
  
    memcpy(copy, &item, sizeof(T));
    tree->insert_single(key, copy);
  } else {
...
void TPCCTables::insertItem(TM_ARGDECL Item* item)
{
  ...
  items_.insert(TM_ARG item->i_id, item);
...
{
  items_.insert_single(item->i_id, item);
}
...
Order* TPCCTables::insertOrderSingleThread(const Order& order)
{
  ...
  // Secondary index based on customer id
  int64_t key = makeOrderByCustomerKey(order.o_w_id, order.o_d_id, order.o_c_id, order.o_id);
  orders_by_customer_.insert_single(key, tuple);
  return tuple;
}
...
__attribute__((transaction_safe)) NewOrder* TPCCTables::insertNewOrder(int64_t w_id, int64_t d_id, int64_t o_id) {
  ...
  FAST_PATH_SHARED_WRITE(neworder->no_w_id, w_id);
  //neworder->no_d_id = d_id;
  FAST_PATH_SHARED_WRITE(neworder->no_d_id, d_id);
  //neworder->no_o_id = o_id;
  FAST_PATH_SHARED_WRITE(neworder->no_o_id, o_id);
...
NewOrder* TPCCTables::insertNewOrderSingleThread(int64_t w_id, int64_t d_id, int64_t o_id)
{
  ...
  int64_t key = makeNewOrderKey(neworder->no_w_id, neworder->no_d_id, neworder->no_o_id);
  neworders_.insert_single(key, neworder);
  return neworder;
}
