...
void TPCCClient::doOne(TM_ARGDECL_ALONE) {
    ...
    unsigned long sumAllTxs = executed_stock_level_txs_ + executed_delivery_txs_ + executed_order_status_txs_ + executed_payment_txs_ + executed_new_order_txs_;
    int x = generator_->number(1, 1000000);

    if (x <= global_stock_level_txs_ratio*10000) {
        doStockLevel(TM_ARG_ALONE);
        executed_stock_level_txs_++;
    } else if (x <= (global_stock_level_txs_ratio + global_delivery_txs_ratio)*10000) {
        doDelivery(TM_ARG_ALONE);
        executed_delivery_txs_++;
    } else if (x <= (global_stock_level_txs_ratio + global_delivery_txs_ratio + global_order_status_txs_ratio)*10000) {
        doOrderStatus(TM_ARG_ALONE);
        executed_order_status_txs_++;
    } else if (x <= (global_stock_level_txs_ratio + global_delivery_txs_ratio + global_order_status_txs_ratio + global_payment_txs_ratio)*10000) {
...
int32_t TPCCClient::generateWarehouse() {
    if (bound_warehouse_ == 0) {
        ...
        // return (tid % global_num_warehouses) + 1;
